...
{
	return data_type == trpc::ThriftDataType::kBool
		|| data_type == trpc::ThriftDataType::kI08
		|| data_type == trpc::ThriftDataType::kI16
		|| data_type == trpc::ThriftDataType::kI32
		|| data_type == trpc::ThriftDataType::kI64
		|| data_type == trpc::ThriftDataType::kU64
		|| data_type == trpc::ThriftDataType::kDouble;
}
...
static void output_descriptor(int& i, FILE *f, const std::string& type_name, size_t& cur, const idl_info& info)
{
	...
	std::string ret;
	if (cpptype == "bool")
		data_type = trpc::ThriftDataType::kBool;
		...
		data_type = trpc::ThriftDataType::kString;
	else if (cpptype == "std::map" && cur < type_name.size() && type_name[cur] == '<')
	{
		data_type = trpc::ThriftDataType::kMap;
	...
	{
		data_type = trpc::ThriftDataType::kSet;
		output_descriptor(i, f, type_name, ++cur, info);
		val_arg = "subtype_" + std::to_string(i);
		cpptype = SGenUtil::strip(type_name.substr(st, ++cur - st));
	}
	...
	{
		data_type = trpc::ThriftDataType::kList;
		output_descriptor(i, f, type_name, ++cur, info);
		val_arg = "subtype_" + std::to_string(i);
		cpptype = SGenUtil::strip(type_name.substr(st, ++cur - st));
	}
	else
		data_type = trpc::ThriftDataType::kStruct;

	fprintf(f, "\t\tusing subtype_%d = trpc::ThriftDescriptorImpl<%s, %d, %s, %s>;\n",
...
public:
	...
	void print_trpc_thrift_header(const std::string& service, const std::vector<rpc_descriptor>& rpcs)
	{
	...
	void print_trpc_svr_ctor(std::string svc_name,std::vector<std::string>& methods,const std::vector<rpc_descriptor>& rpcs, bool async){
		if(async) svc_name = "Async"+svc_name;

		fprintf(this->out_file, "%s::%s(){", svc_name.c_str(),svc_name.c_str());
		int i=0;
		for (const auto& rpc : rpcs){
			auto req_typename =rpc.req_params[0].type_name;
			auto rsp_typename =rpc.resp_params[0].type_name;
			fprintf(this->out_file, trpc_service_server_ctor_reg_method_fmt.data(),
				methods[i].c_str(),async?"Async":"",req_typename.c_str(),rsp_typename.c_str(),svc_name.c_str(),rpc.method_name.c_str(),async?"":",std::placeholders::_3");
			i++;
		}
		fprintf(this->out_file, "}\n");
	}

	void print_class_method_impl(std::string_view fmt,const std::string& service,const std::string& method,const std::string& req,const std::string& rsp){
		fprintf(this->out_file, fmt.data(),service.c_str(),method.c_str(),req.c_str(),rsp.c_str());
	}



	void print_trpc_thrift_source(const std::string& service, const std::vector<rpc_descriptor>& rpcs)
	{
		std::string return_type;
		std::string handler_params;
		std::string send_params;
		...
		fprintf(this->out_file, trpc_service_registered_methods.data(),service.c_str(),ss.str().c_str());
		
		this->print_trpc_svr_ctor(service, methods, rpcs, false);
		this->print_trpc_svr_ctor(service, methods, rpcs, true);
		// 服务端方法
		for(auto& rpc:rpcs){
			auto req_typename =rpc.req_params[0].type_name;
			auto rsp_typename =rpc.resp_params[0].type_name;
	...
	void print_custom_str(const std::string& fmt,const std::string& content){
		fprintf(this->out_file, fmt.c_str(),content.c_str());
	}
...
using %sDone = std::function<void (%s *, trpc::RPCContext *)>;)";

	std::string client_define_task_format = R"(using %sTask = trpc::RPCClientTask<%s, %s>;
)";

	std::string server_context_format = R"(
using %sContext = trpc::RPCContext<%s, %s>;
)";

	std::string server_controller_format = R"(
class Server%sCntl : public trpc::%sCntl
{
public:
	// must implement this in server codes
	void %s(%s *request, %s *response);

	Server%sCntl(trpc::RPCTask *task)
		: %sCntl(task)
	{}
};
)";
...
class Client%sCntl : public trpc::%sCntl
{
public:
	Client%sCntl(trpc::RPCTask *task)
		: %sCntl(task)
	{}
...
void %s(trpc::RPCTask *task, %s *request, %s *response);

static inline void %s%s(trpc::RPCTask *task, \
...
public:
	...
	virtual void %s(%s *request, %s *response,
					trpc::RPCContext *ctx) = 0;
)";

	std::string server_class_method_declaration_thrift_format = R"(
	virtual void %s(::trpc::ServerContextPtr context, %s *request, %s *response);
)";

	std::string server_class_method_implementation_thrift_format = R"(
inline void Service::%s(%s *request, %s *response, trpc::RPCContext *ctx)
{
	%sthis->%s(%s);
}
)";
...
inline %s Service::%s(%s) {%s}
	...
	void %s%s(trpc::RPCTask *task);
)";

	std::string server_constructor_method_format = R"(
inline Service::Service(): trpc::RPCService("%s")
{)";

	std::string server_constructor_add_method_format = R"(
	this->trpc::RPCService::add_method("%s",
		[this](trpc::RPCWorker& worker) ->int {
			return ServiceRPCCallImpl(this, worker, &Service::%s);
		});
)";

	std::string server_methods_format = R"(
void %s::%s%s(trpc::RPCTask *task)
{
	Server%sCntl cntl(task);
	trpc::RPCService::parse_from_buffer(task, cntl.get_input(), true);
	cntl.%s(cntl.get_req(), cntl.get_resp());
	trpc::RPCService::serialize_to_buffer(task, cntl.get_output(), false);
}
)";

	std::string client_class_constructor_start_format = R"(
class %sClient : public trpc::%sClient
{
public:)";
...
inline %sClient::%sClient(const char *host, unsigned short port):
	trpc::%sClient("%s")
{
	struct trpc::RPCClientParams params = trpc::RPC_CLIENT_PARAMS_DEFAULT;
	%s
	params.host = host;
	params.port = port;
	this->trpc::%sClient::init(&params);
}

inline %sClient::%sClient(const struct trpc::RPCClientParams *params):
	trpc::%sClient("%s")
{
	const struct trpc::RPCClientParams *temp = params;
	struct trpc::RPCClientParams temp_params;
	%s
	this->trpc::%sClient::init(temp);
}
)";

	std::string client_constructor_methods_ip_srpc_thrift_format = R"(
	params.task_params.data_type = trpc::RPCDataThrift;
)";

	std::string client_constructor_methods_params_srpc_thrift_format = R"(
	if (params->task_params.data_type == trpc::RPCDataUndefined)
	{
		temp_params = *temp;
		temp_params.task_params.data_type = trpc::RPCDataThrift;
		temp = &temp_params;
	}
)";
			...
			trpc::RPCService("%s")
	{
		...
		this->register_compress_type(trpc::RPCCompressGzip);
)";

	std::string client_class_constructor_params_format = R"(
	%sClient(struct trpc::RPCServiceParams& params) :
			trpc::RPCService("%s")
	{
	...
	const struct trpc::RPCClientParams *get_params()
	{
		return &this->params;
	}
)";
	...
	struct trpc::RPCClientParams params;
)";
	...
	const trpc::RPCSyncContext& thrift_last_sync_ctx() const;
)";

	std::string client_class_private_get_thrift_format = R"(
private:
	static trpc::RPCSyncContext *get_thread_sync_ctx();
)";

	std::string client_class_get_rpc_thread_resp_thrift_format = R"(
	static trpc::ThriftReceiver<%s> *get_thread_sync_receiver_%s();
)";

	std::string client_class_construct_methods_format = R"(
	void %s(const %s *req, %sDone done);
	void %s(const %s *req, %s *resp, trpc::RPCSyncContext *sync_ctx);
	WFFuture<std::pair<%s, trpc::RPCSyncContext>> async_%s(const %s *req);
)";

	std::string client_class_constructor_format = R"(
public:
	%sClient(const char *host, unsigned short port);
	%sClient(const struct trpc::RPCClientParams *params);

public:
)";

	std::string client_class_create_methods_format = R"(	trpc::%sClientTask *create_%s_task(%sDone done);
)";
...
inline void %sClient::%s(const %s *req, %s *resp, trpc::RPCSyncContext *sync_ctx)
{
...
inline WFFuture<std::pair<%s, trpc::RPCSyncContext>> %sClient::async_%s(const %s *req)
{
	using RESULT = std::pair<%s, trpc::RPCSyncContext>;
	auto *pr = new WFPromise<RESULT>();
	auto fr = pr->get_future();
	auto *task = this->create_rpc_client_task<%s>("%s", trpc::RPCAsyncFutureCallback<%s>);
...
inline trpc::ThriftReceiver<%s> *%sClient::get_thread_sync_receiver_%s()
{
	static thread_local trpc::ThriftReceiver<%s> receiver;
	return &receiver;
}
...
inline void %sClient::send_%s(%s)
{
	...
	auto *task = this->create_rpc_client_task<%s>("%s", trpc::ThriftSendCallback<%s>);
...
inline trpc::RPCSyncContext *%sClient::get_thread_sync_ctx()
{
	static thread_local trpc::RPCSyncContext thread_sync_ctx;
...
inline const trpc::RPCSyncContext& %sClient::thrift_last_sync_ctx() const
{
...
inline trpc::%sClientTask *%sClient::create_%s_task(%sDone done)
{
...
public:
	...
	void %s(%s *request, %s *response, trpc::RPCContext *ctx) override
	{
...
static void %s_done(%s *response, trpc::RPCContext *context)
{
}
)";
...
/*
	...
	%s %s_req;
	trpc::RPC%s::%sTask *%s_task = client.create_%s_task(%s_done);
...
class %s : public trpc::ThriftIDLMessage
{
public:
)";
	...
	friend class trpc::ThriftElementsImpl<%s>;
};
)";
	std::string thrift_struct_element_impl_begin_format = R"(
private:
	static void StaticElementsImpl(std::list<trpc::struct_element> *elements)
	{
