...
bool Parser::parse(const std::string& proto_file, idl_info& info)
{
	...
	if (!in)
	{
		fprintf(stderr, "[Parser] proto file: [%s] not exists.\n dir:[%s],file:[%s]",
				idl_file.c_str(),info.input_dir.c_str(),proto_file.c_str());
		return false;
	}
								...
								rpc_param& param, idl_info& info)
{
	if (idl_type == "bool")
	{
		param.data_type = trpc::ThriftDataType::kBool;
		param.type_name = "bool";
	}
	else if (idl_type == "i8" || idl_type == "byte")
	{
		param.data_type = trpc::ThriftDataType::kI08;
		param.type_name = "int8_t";
	}
	else if (idl_type == "i16")
	{
		param.data_type = trpc::ThriftDataType::kI16;
		param.type_name = "int16_t";
	}
	else if (idl_type == "i32")
	{
		param.data_type = trpc::ThriftDataType::kI32;
		param.type_name = "int32_t";
	}
	else if (idl_type == "i64")
	{
		param.data_type = trpc::ThriftDataType::kI64;
		param.type_name = "int64_t";
	}
	else if (idl_type == "u64")
	{
		param.data_type = trpc::ThriftDataType::kU64;
		param.type_name = "uint64_t";
	}
	else if (idl_type == "double")
	{
		param.data_type = trpc::ThriftDataType::kDouble;
		param.type_name = "double";
	}
	else if (idl_type == "string" || idl_type == "binary")
	{
		param.data_type = trpc::ThriftDataType::kString;
		param.type_name = "std::string";
	}
	else if (SGenUtil::start_with(idl_type, "list"))
	{
		size_t cur = 0;
		param.data_type = trpc::ThriftDataType::kList;
	...
	else
	{
		...
		{
			//struct
			param.data_type = trpc::ThriftDataType::kStruct;
									...
									idl_info& info)
{
	...
	if (left_b + 1 < str.size())
	{
		...
		for (const auto& ele : bb)
		{
			...
			param.required_state = trpc::kThriftStructFieldRequired;
								 ...
								 Descriptor& desc, idl_info& info)
{
	...
	for (const auto& ele : arr)
	{
		...
		{
			param.required_state = trpc::kThriftStructFieldDefault;
			b1 = "default";
		...
		else
		{
			param.required_state = (b1 == "required") ?
								   trpc::kThriftStructFieldRequired :
								   trpc::kThriftStructFieldOptional;
